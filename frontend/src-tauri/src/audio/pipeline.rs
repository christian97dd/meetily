...
pub struct AudioPipeline {
    ...
    state: Arc<RecordingState>,
    // One VAD per source so each transcript segment keeps who it came from (mic = user, system = others).
    // Both are fed equal-length windows from the ring buffer, so their timestamps share one timeline.
...
impl AudioPipeline {
    pub fn new(
        ...
        let system_vad = ContinuousVadProcessor::new(sample_rate, VAD_REDEMPTION_TIME_MS)?;
        info!(
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // STEP 3: Transcribe each source separately (VAD + Whisper)
                            match self.mic_vad.process_audio(&mic_window) {
                                Ok(segments) => self.send_speech_segments(segments, DeviceType::Microphone),
                                Err(e) => warn!("⚠️ Mic VAD error: {}", e),
                            }
                            match self.system_vad.process_audio(&sys_window) {
                                Ok(segments) => self.send_speech_segments(segments, DeviceType::System),
