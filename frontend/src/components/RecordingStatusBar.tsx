...
export const RecordingStatusBar: React.FC<RecordingStatusBarProps> = ({ isPaused = false }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-2 px-3 py-2 bg-muted/50 rounded-lg mb-2"
    >
      <div className={`w-2 h-2 rounded-full ${isPaused ? 'bg-orange-500' : 'bg-red-500 animate-pulse'}`} />
      <span className={`text-sm ${isPaused ? 'text-orange-700 dark:text-orange-300' : 'text-foreground/80'}`}>
