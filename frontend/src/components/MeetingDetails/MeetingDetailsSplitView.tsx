  ...
  return (
    ...
    >
          ...
          {TABS.map((tab) => {
            ...
            return (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="relative z-10 flex items-center gap-2 rounded-none border-0 bg-transparent px-6 py-4 text-muted-foreground data-[state=active]:bg-transparent data-[state=active]:text-blue-600 data-[state=active]:shadow-none hover:text-foreground"
              >
      ...
      >
        ...
        >
          <div className="h-full w-px bg-muted-strong transition-[width,background-color] duration-150 ease-out group-hover:w-1 group-hover:bg-blue-400 group-active:w-1 group-active:bg-blue-500" />
        </div>
        <TabsContent
          value="summary"
          forceMount
