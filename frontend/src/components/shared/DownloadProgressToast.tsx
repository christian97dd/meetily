...
function DownloadToastContent({
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-background rounded-lg shadow-lg border border-border p-3 relative">

      {/* Icon */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-green-100 dark:bg-green-900/40' : hasError ? 'bg-red-100 dark:bg-red-900/40' : isCancelled ? 'bg-muted' : 'bg-muted'
        }`}>
        {isComplete ? (
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasError ? (
          ...
          <>
            {/* Progress bar */}
            <div className="w-full h-1.5 bg-muted-strong rounded-full overflow-hidden mb-1.5">
              <div
