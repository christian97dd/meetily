  ...
  return (
    <div 
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-blue-50 dark:bg-blue-950/40 ring-1 ring-blue-200 dark:ring-blue-800 shadow-sm' : 'hover:bg-muted/50'}`}
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-muted/50
                  ${index === selectedCommandIndex ? 'bg-muted/50' : ''}
                `}
