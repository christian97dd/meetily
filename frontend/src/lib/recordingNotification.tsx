      ...
      const toastId = toast.info('🔴 Recording Started', {
        description: (
          <div className="space-y-3 min-w-[280px]">
            ...
            <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/60 p-2 rounded transition-colors">
              <input
                type="checkbox"
                onChange={(e) => {
                  dontShowAgain = e.target.checked;
                }}
                className="rounded border-input text-blue-600 dark:text-blue-400 focus:ring-blue-500 focus:ring-2"
              />
              <span className="select-none text-foreground/80">Don't show this again</span>
            </label>
            <button
              onClick={async () => {
                if (dontShowAgain) {
